...
from CUmodel_scipy import CUmodelS
...
def train_one(data_name, n_pairs, data_config, CU_config, TaylorCU_config, random_seed):
    ...
    cdf_func, dense_func, cdf_func_T = get_CDF_and_dense(data_name, data_config, data_loader.y)
    ...
    mdl1 = CUmodel()
    mdl1.fit(X_plus,X_minus,cdf_func,data_loader.X,cdf_func_T, **CU_config)
    ...
    mdl3 = CUmodelS()
    mdl3.fit(X_plus,X_minus,cdf_func,data_loader.X)
    err4 = data_loader.y - mdl3.predict_val(data_loader.X, lowerS = min_y, upperS = max_y)
    
    logger.debug((np.mean(err*err),np.mean(err1*err1), np.mean(err2*err2), np.mean(err3*err3),np.mean(err4*err4)))
    logger.debug(mdl1.model.weight._parameters)
    logger.debug(mdl2.model._parameters)
